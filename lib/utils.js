...
module.exports = class AccessUtils {
  ...
  setupFilters() {
    const models = [
      ...
      this.options.userModel,
    ].concat(this.getGroupContentModels());

    models.forEach(modelName => {
      const Model = this.app.models[modelName];

      debug('Attaching remote hook to %s', modelName);

      // TODO: Attach to all methods?
      Model.beforeRemote('**', (ctx, instance, next) => {
        ...
        const whereFilter = args.filter ? args.filter.where : args.where;
        ...
        if (this.options.allowSkipAccess && ctx.options.skipAccess) {
          debug('skipAccess: true - skipping access filters');
          return next();
        }

        // Do not filter if the request is being made against a single model instance.
        if (whereFilter && whereFilter.hasOwnProperty('id')) {
          debug('looking up by Id - skipping access filters');
          return next();
        }

        // Do not apply filters if no group access acls were applied.
        if (!ctx.req.groupAccessApplied) {
          debug('acls not appled - skipping access filters');
          return next();
        }

        debug('%s remote hook "%s": args=%s, options=%o',
          Model.modelName, ctx.method.name, JSON.stringify(args, null, 4), ctx.options);

        this.buildFilter(currentUser.id, ctx.Model)
          .then(filter => {
            debug('original args: %s', JSON.stringify(args, null, 4));
            const where = whereFilter ? {
              and: [ whereFilter, filter ]
            } : filter;

            // TODO: some methods use a "where" arg and others like find use "filter" arg. Below handles that but doesn't work when no args are provided. Need to add correct arg based on method type?
            if (args.filter) {
            ...
            ctx.args = args;
            debug('modified args: %s', JSON.stringify(args, null, 4));
            next();
          })
          .catch(next);
      });
  ...
  getUserGroups(userId, force, cb) {
    ...
    const groupAccessModel = this.app.models[this.options.groupAccessModel];
    let foreignKey = 'userId';

    Object.keys(groupAccessModel.relations).forEach(relationName => {
      const relation = groupAccessModel.relations[relationName];

      if (relation.modelTo.definition.name === this.options.userModel) {
  ...
  setupRoleResolver(accessGroup) {
    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      ...
      // Set group access applied context
      context.remotingContext.req.groupAccessApplied = true;
