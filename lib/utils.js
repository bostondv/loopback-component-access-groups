    ...
    models.forEach(modelName => {
      ...
      Model.beforeRemote('*', (ctx, unused, next) => {
        ...
        if (ctx.method.name.indexOf('find') > -1) {
          try {
            argsFilter = JSON.parse(args.filter) || {};
          } catch(err) {
        ...
        if (ctx.args.hasOwnProperty('id') ||
            argsFilter.where && argsFilter.where.hasOwnProperty('id')) {
        ...
        debug('%s remote hook "%s": where=%o, options=%o',
          Model.modelName, ctx.method.name, argsFilter.where, ctx.options);

        this.buildFilter(currentUser.id, ctx.Model)
          .then(filter => {
            debug('original where: %o', argsFilter.where);
            const where = argsFilter.where ? {
              and: [ argsFilter.where, filter ]
            } : filter;

            argsFilter.where = where;

            if (ctx.method.name.indexOf('find') > -1) {
              ctx.args.filter = argsFilter;
            } else {
              ctx.args = argsFilter;
            }
            debug('modified where: %o', argsFilter.where);
            next();
          })
          .catch(next);
      });
